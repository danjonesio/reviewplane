...
It runs one workspace package at a time (`--workspace-concurrency=1`). At pnpm's default concurrency a failing suite stops reporting altogether on a hosted runner: the run stalls until the job timeout, and `node --test` never reaches its summary, so the log carries failure marks with no error attached to any of them. Serially the same failure is reported in about ninety seconds with its stack. The bound buys a report, not a pass — the suites are not yet isolated from each other or from their own fixtures, which RVP-62 owns — and until they are, the gate MUST be the invocation that reports honestly.
