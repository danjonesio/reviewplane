...
Working today at the repository root: `pnpm install`, `pnpm build`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm test:browser`, `pnpm test:ui`, `pnpm test:integration`, `pnpm test:e2e`, `pnpm test:edge`, `pnpm test:install`, `pnpm test:faults`, `pnpm audit:dependencies`, `pnpm protocol:generate` and `pnpm protocol:check`. `go test ./...`, `go test -race ./...` and `go vet ./...` run from a module directory: `packages/protocol`, `services/connector` or `services/tunnel-gateway`. The remaining scripts arrive with the surfaces they exercise.
...
`pnpm test:install` runs `deploy/compose/e2e/install-smoke.sh`: it materialises a clean copy of the repository — `git ls-files`, so no `node_modules`, no build output and no generated secret — and runs `docs/DEPLOYMENT.md` §8's commands **verbatim** in it, then asserts what the installation is supposed to be true of. It reaches a login page over TLS through the gateway, drives `/api` and `/mcp` to their real upstreams, port-scans the host for PostgreSQL and the Chromium debugging port, inspects every container for a Docker socket, reads the browser worker's uid and its registered sandbox posture, and injects three faults: PostgreSQL stopped, an unwritable artefact store and a missing secret file. It is the gate for the Stage 1 exit criterion "fresh installation **from a clean checkout** in one documented flow" (`docs/ROADMAP.md` §3, amended by RVP-97 so that the criterion and the suite meeting it say the same thing), and it needs Docker and roughly twenty minutes on a small host, because it builds every image. Evidence lands in `deploy/compose/e2e/evidence-install/`.

`pnpm test:faults` runs `deploy/compose/e2e/fault-injection.sh`: the six rows of the `docs/TESTING.md` §11 fault-injection matrix that can only be answered against a deployed stack. It brings up the Compose stack, publishes a service, allocates a browser session and then breaks things on purpose — stops the development environment mid-navigation, restarts the control plane under a live WebSocket viewer, stops PostgreSQL, makes the artefact volume unwritable, submits two identical verifications at once, and `SIGKILL`s the browser worker after an upload. Retention deletion is recorded as not applicable in Stage 1 **and asserted to be**, so the assertion fails the day retention arrives. Every failure writes the service logs, an event dump and an artefact inventory into `deploy/compose/e2e/evidence-faults/`, beside a report naming each case's verdict and the bounds the run applied. It needs Docker and roughly fifteen minutes, most of it the shipped ninety-second worker-loss bound, which is used rather than shortened so the case measures the deployment an operator runs.

`pnpm audit:dependencies` runs `.github/scripts/dependency-audit.mjs`, the owner of the `docs/TESTING.md` §16 release condition "critical dependency vulnerability lacks documented mitigation". It queries the npm advisory endpoint through `pnpm audit` and `govulncheck` over each Go module, and **fails closed**: an unreachable or unparseable source, and a Go vulnerability database older than thirty days, are failures rather than passes. It therefore needs network access and a `govulncheck` on `PATH` (or named by `GOVULNCHECK`); `REVIEWPLANE_AUDIT_REGISTRY` points the npm half at an internal mirror. A critical advisory blocks unless `.github/dependency-mitigations.json` carries a current entry for it, and §16.2 of `docs/TESTING.md` records exactly what the gate does and does not look at.
...
`.github/workflows/container-harnesses.yml` runs the container harnesses — `pnpm test:browser`, `pnpm test:ui`, `pnpm test:integration`, `pnpm test:e2e`, `pnpm test:edge` and `pnpm test:install` — on every pull request whose change is not documentation-only, nightly against `main`, and on demand. A pull request is documentation-only when every file it changes is a Markdown document or lives under `docs/`; every other change runs all six, because every other path can alter what they do. They cost minutes rather than seconds, so a documentation change is not made to wait for them, and nothing else is exempt: RVP-73 records what the previous label-only trigger cost, which was a request-schema change merging unharnessed and leaving `pnpm test:integration` failing on `main`. The `ci:harnesses` label still works, now as a manual override that forces the harnesses onto a pull request the filter exempted.

The workflow decides that in a `scope` job rather than in an `on.pull_request.paths` condition, so that it runs — and reports — on every pull request. Its `Harness gates` job is green only when every harness the change required succeeded, or when the change was documentation-only and the run says which files made it so; a harness that should have run and did not is a red check rather than an absence. `docs/TESTING.md` section 16.1 holds the full trigger table for both workflows. A change MUST NOT be merged with a failing gate or a failing harness.

`.github/workflows/release.yml` is the third workflow and answers a different question: not "may this change land" but "may this be released". It runs on a `v*` tag, on demand and nightly, has no path filter at all, and reports one rolled-up `Release gates` check that is green only when every condition of `docs/TESTING.md` section 16 held — with a **skipped** job counted as a failure, because a condition that did not run has not been shown to hold. Beside those it runs `pnpm test:faults` and publishes the section 12 performance baselines, which are recorded and never gated. Section 16 of `docs/TESTING.md` describes what it does; section 16.4 records what a green check does not mean.
