...
a `preHandler`, so a cookie session must present its token **before the body is
decoded**, and a bearer credential carries none and needs none. Every lookup
carries the identifier, the caller's organisation and the session's project
scope in one predicate, so a route or a project outside the caller's scope
