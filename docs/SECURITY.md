...
- `deploy/compose/browser-worker-seccomp.json` is Docker's default profile with one change: creating a user namespace no longer requires `CAP_SYS_ADMIN`, because Chromium's sandbox is built on user namespaces. Every other gate is unchanged.
