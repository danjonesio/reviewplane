...
if [[ ! -s "${TLS}/gateway.crt" || "${FORCE}" -eq 1 ]]; then
  ...
  openssl req -x509 -newkey ec -pkeyopt ec_paramgen_curve:prime256v1 \
    -keyout "${TLS}/tunnel-ca.key" -out "${TLS}/tunnel-ca.pem" \
...
EXT
  ...
  openssl x509 -req -in "${TLS}/gateway.csr" \
    -CA "${TLS}/tunnel-ca.pem" -CAkey "${TLS}/tunnel-ca.key" -CAcreateserial \
    -out "${TLS}/gateway.crt" -days 365 -sha256 \
    -extfile "${TLS}/gateway.ext" 2> /dev/null

  rm -f "${TLS}/gateway.csr" "${TLS}/gateway.ext" "${TLS}/tunnel-ca.srl"
  ...
  chmod 600 "${TLS}/tunnel-ca.key"
  echo "wrote tls/tunnel-ca.pem, tls/gateway.crt and tls/gateway.key"
fi
