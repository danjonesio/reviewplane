...
import { createPool, migrateDatabase } from "../src/db/pool.ts";
...
describe("published-service endpoints", () => {
  ...
  test("a gateway refusal marks the record failed and is audited", async () => {
    const { ApiError } = await import("../src/modules/published-services/errors.ts");
    gateway.refuseWith = new ApiError(422, "DESTINATION_NOT_ALLOWED", "refused by the gateway");
    try {
      const response = await request(
        "POST",
