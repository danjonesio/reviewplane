...
export class HttpTunnelGateway implements TunnelGateway {
  ...
  async register(request: GatewayRegisterRequest): Promise<GatewayRouteView> {
    ...
    if (!response.ok || body.data === undefined) {
      ...
      // what keeps one failure one code all the way to the caller.
      throw new ApiError(
        422,
  ...
  async revokeRoute(routeId: string): Promise<void> {
    ...
    if (!response.ok && response.status !== 404) {
      throw new ApiError(502, "INTERNAL_ERROR", "The tunnel gateway could not revoke this route.");
    }
  }
  ...
  async #send(method: string, path: string, body?: unknown): Promise<Response> {
    ...
    try {
      ...
      throw new ApiError(503, "CONNECTOR_OFFLINE", "The tunnel gateway is not reachable.", {
