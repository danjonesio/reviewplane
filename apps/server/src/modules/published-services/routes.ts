...
function requireString(value: unknown, field: string): string {
  ...
  if (value.length > 64) {
    throw new ApiError(422, "VALIDATION_FAILED", `${field} is too long.`, { field });
  }
  return value;
}
...
function requirePort(value: unknown): number {
  if (typeof value !== "number" || !Number.isInteger(value) || value < 1 || value > 65535) {
    throw new ApiError(422, "VALIDATION_FAILED", "local_port must be a port number.", {
      field: "local_port",
    });
  }
  return value;
...
function requireSessionIds(value: unknown): string[] {
  if (!Array.isArray(value)) {
    throw new ApiError(422, "VALIDATION_FAILED", "allowed_browser_session_ids is required.", {
