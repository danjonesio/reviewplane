...
import type { Database, DatabaseClient } from "../../db/pool.ts";
...
export class PublishedServiceService {
  ...
  async create(
    ...
    if (input.ttlSeconds <= 0 || input.ttlSeconds > this.#config.routeTtlMaxSeconds) {
      throw new ApiError(422, "ROUTE_EXPIRED", "The requested route lifetime is not permitted.", {
    ...
    try {
      ...
      return await inTransaction(this.#pool, async (client) => {
        ...
        if (ready === null) {
          throw new ApiError(409, "PUBLISHED_SERVICE_UNAVAILABLE", "This route is no longer pending.");
        }
        await recordEvent(client, {
  ...
  async mint(
    ...
    if (requested <= 0 || requested > this.#config.capabilityTtlSeconds) {
      throw new ApiError(422, "VALIDATION_FAILED", "The requested capability lifetime is not permitted.", {
    ...
    if (expiresAt <= now) {
      throw new ApiError(409, "ROUTE_EXPIRED", "This published service has expired.");
    }
  ...
  async #read(serviceId: string): Promise<PublishedService> {
    ...
    try {
      ...
      if (service === null) {
        throw new ApiError(404, "RESOURCE_NOT_FOUND", "No such published service.");
      }
      return service;
    } finally {
      client.release();
