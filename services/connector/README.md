...
Route publication and data streams, workspace discovery and Git context, the
local MCP bridge, local notifications, self-update, and systemd unit packaging.
A `route.publish` arriving today is logged and left unacknowledged rather than
answered with an error class that would misdescribe the reason.
