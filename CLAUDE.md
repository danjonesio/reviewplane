...
`reviewplane` is the operator command line shipped in the server image: `reviewplane migrate` (and `migrate --status`, `migrate --preflight` for the upgrade preflight of `docs/OPERATIONS.md` §12), `reviewplane serve`, `reviewplane jobs`, `reviewplane install-token` (the one-time administrator bootstrap token), `reviewplane status [--json]` (the deployment's health, capacity, storage, backup age and certificate expiry), `reviewplane connector list` (enrolled connectors and their health), `reviewplane backup --output FILE|-` and `reviewplane restore --input FILE|-` (one archive of the installation and its restoration into an empty one, `docs/DEPLOYMENT.md` §16–§17), `reviewplane export-review` (one review as the portable document of `docs/REVIEW_FORMAT.md`), `reviewplane version` (`docs/DEPLOYMENT.md` §11). In a deployment it is run through `deploy/compose/reviewplane`, which execs it in the `api` container; `backup` and `restore` stream through it, so `./reviewplane backup --output - > archive.tar.zst` lands the archive on the host.

Root commands (`docs/DEVELOPMENT.md` §5): `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, `pnpm protocol:check`. `pnpm typecheck` and `pnpm test` work on a fresh clone with no prior build. Container harnesses: `pnpm test:browser` (Chromium), `pnpm test:ui` (annotation UI), `pnpm test:integration` (steps 9–12 of `docs/TESTING.md` §3, with a real browser worker and a real MCP client), `pnpm test:e2e` (steps 1–6 plus the tunnel capabilities of `docs/ARCHITECTURE.md` §7.4, over the Compose stack), `pnpm test:edge` (the edge gateway's own TLS, document, refusal and header behaviour, which every other suite runs behind rather than through) and `pnpm test:install` (`docs/DEPLOYMENT.md` §8 run verbatim from a clean checkout, ending at a login page over the gateway, plus the §20 negative checks). Release-only commands: `pnpm test:faults` (the `docs/TESTING.md` §11 fault-injection matrix against the deployed stack) and `pnpm audit:dependencies` (the §16 dependency-vulnerability gate, which needs network access and fails closed without it). `go vet ./...`, `go test ./...` and `go test -race ./...` run from a Go module directory: `packages/protocol`, `services/connector` or `services/tunnel-gateway`. `pnpm test` and the server suites need Docker for a disposable PostgreSQL.
