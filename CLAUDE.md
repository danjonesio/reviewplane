...
`reviewplane` is the operator command line shipped in the server image: `reviewplane migrate` (and `migrate --status`), `reviewplane serve`, `reviewplane jobs`, `reviewplane install-token` (the one-time administrator bootstrap token), `reviewplane export-review` (one review as the portable document of `docs/REVIEW_FORMAT.md`), `reviewplane version` (`docs/DEPLOYMENT.md` §11).
